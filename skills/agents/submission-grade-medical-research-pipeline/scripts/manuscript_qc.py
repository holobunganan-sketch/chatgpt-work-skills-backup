...
NON_BLACK_COLOR = re.compile(r'<w:color\b[^>]*w:val="([0-9A-Fa-f]{6}|auto)"')
SUPERSCRIPT_CITATION_RUN = re.compile(
    r'<w:r\b.*?>.*?<w:vertAlign\b[^>]*w:val="superscript"\s*/>.*?<w:t[^>]*>\s*[\[\(]?\d+(?:[-,]\d+)*[\]\)]?\s*</w:t>.*?</w:r>',
    re.S,
)
...
def extract_paragraphs_from_xml(xml: str) -> list[str]:
...
def extract_paragraphs(path: Path) -> list[str]:
    return extract_paragraphs_from_xml(read_document_xml(path))
...
def main() -> int:
    ...
    document_xml = read_document_xml(path)
    ...
    if not SUPERSCRIPT_CITATION_RUN.search(document_xml):
        failures.append("No superscript AMA-style citation runs detected in document XML")

    text_colors = {
        value.lower()
        for value in NON_BLACK_COLOR.findall(document_xml)
        if value.lower() not in {"000000", "auto"}
    }
    if text_colors:
        failures.append(f"Non-black font colors detected in manuscript body: {sorted(text_colors)}")

    table_count = len(re.findall(r"<w:tbl\b", document_xml))
    if table_count == 0:
        failures.append("No actual Word tables detected in manuscript body")
