#!/usr/bin/env python3
"""Scaffold a submission-grade scientific research project tree."""
...
- Mark any missing citation as `citation needed`, `source pending`, or `standard pending`.
...
def main() -> int:
    ...
    parser.add_argument("--topic", default="TBD scientific research topic")
    ...
    write_text(root / "methods/method_details.md", "# Method Details\n\nTBD\n")
    write_text(root / "methods/reproducibility_notes.md", "# Reproducibility Notes\n\nTBD\n")
    write_text(root / "methods/modeling_strategy.md", "# Modeling Strategy\n\nTBD\n")
    ...
    write_text(
        ...
        "source_variable,analysis_variable,definition,unit_or_scale,notes\n",
    )
    write_text(root / "constructed_dataset_spec.md", "# Constructed Dataset Specification\n\nTBD\n")
    write_text(root / "proposed_variable_dictionary.csv", "variable_name,label,type,unit_or_scale,allowed_values,notes\n")
    write_text(root / "literature/cited_papers_log.md", "# Cited Papers Log\n\n")
    write_text(
        root / "literature/evidence_matrix.csv",
        "citation_key,role,claim_supported,study_design_or_type,domain,doi,year,notes\n",
    )
    write_text(
        root / "data/data_dictionary.csv",
        "variable_name,label,type,unit_or_scale,allowed_values,missing_rule,source\n",
    )
    ...
    print(f"Created scientific research scaffold at: {root}")
    return 0
