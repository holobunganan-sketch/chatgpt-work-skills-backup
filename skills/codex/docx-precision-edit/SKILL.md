...
description: Make safe localized edits to a .docx file while preserving embedded "protected elements" (citation fields, superscript cross-references, bookmarks, comments, tracked changes, content controls, equation objects, custom XML, and other machine-managed structures). This skill should be used when the task involves editing an already-finalized Word document whose content and structure are complete but only needs localized language polishing, terminology unification, phrasing rewrites, or logic fixes — and where disturbing embedded reference fields, cross-references, or document machinery would cause correctness or integrity problems (e.g., academic manuscripts with Zotero/EndNote citation fields, legal contracts with clause cross-references, technical specs with variable placeholders and tracked changes, regulatory filings with bookmarks and content controls). It is also appropriate when the work needs to proceed in multiple progressive rounds (word-level normalization → translation-tone removal → logic rewrites), each round a separate copy with full auditability and rollback. Do NOT use this skill for creating new documents from scratch, for wholesale rewrites that change structure, or for tasks where no embedded protected elements exist (plain text edits can use simpler tools).
---
